...
const props = defineProps<{
    balita: {
        data: {
            ...
            nama_orang_tua: string; tgl_lahir: string; jenis_kelamin: string;
            no_telp: string; alamat: string; keterangan: string | null; is_active: boolean;
        }
    }
}>();
...
const form = useForm({
    ...
    nama_orang_tua: d.nama_orang_tua, tgl_lahir: d.tgl_lahir,
    jenis_kelamin: d.jenis_kelamin, no_telp: d.no_telp,
    alamat: d.alamat, keterangan: d.keterangan || '',
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="text-xl font-bold tracking-tight text-gray-900">Edit Balita</h2>
                ...
                <form @submit.prevent="submit" class="space-y-5">
                    ...
                    <Card class="border-none shadow-sm bg-white">
                        ...
                        <CardContent class="p-5 flex flex-col sm:flex-row items-center gap-5">
                            <div class="h-20 w-20 rounded-full overflow-hidden bg-blue-50 flex items-center justify-center border-4 border-white shadow shrink-0">
                                <img v-if="d.foto && !form.foto" :src="d.foto" :alt="d.nama" class="h-full w-full object-cover" />
                                <Baby v-else class="h-8 w-8 text-blue-400" />
                            </div>
                    ...
                    <Card class="border-none shadow-sm bg-white">
                        ...
                        <CardContent class="p-5">
                            <Textarea v-model="form.keterangan" rows="3" class="resize-none"
                                placeholder="Contoh: Riwayat imunisasi, alergi, dll."
