...
import { ArrowLeft, Baby, Calendar, Edit, MapPin, Phone, Trash, User } from 'lucide-vue-next';
...
const chartData = computed(() => ({
    ...
    datasets: [
        ...
        },
        {
            ...
            borderColor: '#10b981',
            data: props.pemeriksaan.data.map(p => p.tinggi_badan),
            tension: 0.3
        }
    ]
}));
...
const deleteBalita = () => {
    if (confirm('Apakah Anda yakin ingin menghapus data ini?')) {
        router.delete(route('balita.destroy', { balita: data.id }));
    }
};
</script>
        ...
        <template #header>
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-4">
                    ...
                    <h2 class="font-semibold text-xl text-gray-800 leading-tight">Detail Balita</h2>
                </div>
                <div class="flex items-center gap-2">
                    <Link :href="route('balita.edit', { balita: data.id })">
                        <Button variant="outline" class="flex items-center gap-2">
                            <Edit class="h-4 w-4" />
                            Edit
                        </Button>
                    </Link>
                    <Button variant="destructive" class="flex items-center gap-2" @click="deleteBalita">
                        <Trash class="h-4 w-4" />
                        Hapus
                    </Button>
                </div>
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <Card class="md:col-span-2">
                        <CardHeader>
                            ...
                            <CardDescription>Detail lengkap riwayat dan kondisi balita.</CardDescription>
                        </CardHeader>
                        <CardContent class="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-4">
                            <div class="flex items-start gap-3">
                                <User class="h-5 w-5 text-muted-foreground mt-0.5" />
                                <div>
                                    <p class="text-sm font-medium">Orang Tua / Wali</p>
                                    <p class="text-sm text-muted-foreground">{{ data.nama_orang_tua }}</p>
                                </div>
                            </div>

                            <div class="flex items-start gap-3">
                                <Baby class="h-5 w-5 text-muted-foreground mt-0.5" />
                                <div>
                                    <p class="text-sm font-medium">Jenis Kelamin</p>
                                    <p class="text-sm text-muted-foreground">{{ data.jenis_kelamin === 'L' ? 'Laki-laki' : 'Perempuan' }}</p>
                                </div>
                            </div>
                            ...
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    <p class="text-sm font-medium">No. Telepon Orang Tua</p>
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Tren berat badan dan tinggi badan dari waktu ke waktu.</CardDescription>
                    </CardHeader>
                    <CardContent>
                ...
                <Card>
                    <CardHeader class="flex flex-row items-center justify-between">
                        <div>
                            ...
                            <CardDescription>Semua catatan kesehatan balita.</CardDescription>
                        </div>
                        <Link :href="route('pemeriksaan.create', { type: 'balita', id: data.id })">
                            ...
                            <table class="w-full text-sm">
                                <thead>
                                    <tr class="bg-muted/50 border-b">
                                        ...
                                        <th class="px-4 py-3 text-left font-medium">TB (cm)</th>
                                        <th class="px-4 py-3 text-left font-medium">L. Kepala</th>
                                ...
                                <tbody>
                                    <tr v-for="item in pemeriksaan.data" :key="item.id" class="border-b transition-colors hover:bg-muted/30">
                                        ...
                                        <td class="px-4 py-3">{{ item.tinggi_badan || '-' }}</td>
                                        <td class="px-4 py-3">{{ item.lingkar_kepala || '-' }}</td>
                                        <td class="px-4 py-3">
                                            <Badge :variant="item.hadir ? 'default' : 'destructive'">{{ item.hadir ? 'Hadir' : 'Absen' }}</Badge>
                                        </td>
                ...
                <Card v-if="data.keterangan">
                    <CardHeader>
                        <CardTitle>Keterangan / Catatan Tambahan</CardTitle>
                    </CardHeader>
                    <CardContent>
