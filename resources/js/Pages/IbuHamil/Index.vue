...
watch(search, debounce((value: string) => {
    router.get(route('ibu-hamil.index'), { search: value }, { preserveState: true, replace: true })
}, 500))
        ...
        <div class="space-y-4">
                ...
                <Table>
                    <TableHeader>
                        <TableRow>
                            ...
                            <TableHead class="text-center">Usia Hamil</TableHead>
                    ...
                    <TableBody>
                        <TableRow v-for="item in ibu_hamil.data" :key="item.id">
                            ...
                            <TableCell class="text-center">
                                <Badge variant="secondary">{{ item.usia_kehamilan }} Minggu</Badge>
                            </TableCell>
                            ...
                            <TableCell>
                                <DropdownMenu>
                                    ...
                                    <DropdownMenuContent align="end">
                                        <DropdownMenuItem @click="router.get(route('ibu-hamil.show', item.id))">Lihat Detail</DropdownMenuItem>
                                        <DropdownMenuItem @click="router.get(route('ibu-hamil.edit', item.id))">Edit Data</DropdownMenuItem>
                                        <DropdownMenuSeparator />
                                        <DropdownMenuItem class="text-destructive" @click="deleteIbuHamil(item.id)">Hapus</DropdownMenuItem>
                                    </DropdownMenuContent>
                                </DropdownMenu>
                            </TableCell>
                        </TableRow>
            ...
            <div class="flex items-center justify-between text-sm">
                <p class="text-muted-foreground">
                    Menampilkan {{ ibu_hamil.data.length }} dari {{ ibu_hamil.meta?.total }} data
                </p>
                <div class="flex items-center gap-1">
                    <Link
                        v-for="link in ibu_hamil.meta?.links" :key="link.label" :href="link.url || '#'"
