        ...
        <div class="space-y-4">
                ...
                <Table>
                    ...
                    <TableBody>
                        <TableRow v-for="item in jadwal.data" :key="item.id">
                            <TableCell>
                                <div>
                                    <p class="font-medium">{{ item.tanggal }}</p>
                                    <p class="text-xs text-muted-foreground">{{ item.waktu_mulai }} - {{ item.waktu_selesai || 'Selesai' }}</p>
                                </div>
                            </TableCell>
                            <TableCell class="font-medium">{{ item.posyandu.nama }}</TableCell>
                            <TableCell class="text-muted-foreground">{{ item.kader?.nama || '-' }}</TableCell>
                            <TableCell class="text-muted-foreground">{{ item.bidan?.nama || '-' }}</TableCell>
                            ...
                            <TableCell>
                                <DropdownMenu>
                                    ...
                                    <DropdownMenuContent align="end">
                                        <DropdownMenuItem @click="router.get(route('jadwal-posyandu.edit', item.id))">Edit / Validasi</DropdownMenuItem>
                                        <DropdownMenuSeparator />
                                        <DropdownMenuItem class="text-destructive" @click="deleteJadwal(item.id)">Hapus</DropdownMenuItem>
                                    </DropdownMenuContent>
                                </DropdownMenu>
                            </TableCell>
                        </TableRow>
            ...
            <div class="flex items-center justify-between text-sm">
                <p class="text-muted-foreground">
                    Menampilkan {{ jadwal.data.length }} dari {{ jadwal.meta?.total }} data
                </p>
                <div class="flex items-center gap-1">
                    <Link
                        v-for="link in jadwal.meta?.links" :key="link.label" :href="link.url || '#'"
