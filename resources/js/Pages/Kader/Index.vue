...
watch(search, debounce((value: string) => {
    router.get(route('kader.index'), { search: value }, { preserveState: true, replace: true })
}, 500))
        ...
        <div class="space-y-4">
                ...
                <Table>
                    ...
                    <TableBody>
                        <TableRow v-for="item in kader.data" :key="item.id">
                            ...
                            <TableCell class="text-muted-foreground">{{ item.posyandu_nama }}</TableCell>
                            ...
                            <TableCell>
                                <DropdownMenu>
                                    ...
                                    <DropdownMenuContent align="end">
                                        <DropdownMenuItem @click="router.get(route('kader.show', item.id))">Lihat Detail</DropdownMenuItem>
                                        <DropdownMenuItem @click="router.get(route('kader.edit', item.id))">Edit Data</DropdownMenuItem>
                                        <DropdownMenuSeparator />
                                        <DropdownMenuItem class="text-destructive" @click="deleteKader(item.id)">Hapus</DropdownMenuItem>
                                    </DropdownMenuContent>
                                </DropdownMenu>
                            </TableCell>
                        </TableRow>
            ...
            <div class="flex items-center justify-between text-sm">
                <p class="text-muted-foreground">
                    Menampilkan {{ kader.data.length }} dari {{ kader.meta?.total }} data
                </p>
                <div class="flex items-center gap-1">
                    <Link
                        v-for="link in kader.meta?.links" :key="link.label" :href="link.url || '#'"
