...
interface Lansia {
    ...
    tgl_lahir: string; umur_label: string; jenis_kelamin: string; is_active: boolean;
}
...
const totalAktif = computed(() => props.lansia.data.filter(i => i.is_active).length);

let timer: ReturnType<typeof setTimeout> | null = null;
watch(search, () => {
    if (timer) clearTimeout(timer);
    timer = setTimeout(() => {
        router.get(route('lansia.index'), { search: search.value || undefined }, { preserveState: true, replace: true });
    }, 400);
});
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="text-xl font-bold tracking-tight text-gray-900">Data Lansia</h2>
                    <p class="text-sm text-muted-foreground mt-0.5">Kelola peserta lansia Posyandu Desa Belumbang</p>
                </div>
                <Button @click="router.get(route('lansia.create'))"
                    class="bg-orange-500 hover:bg-orange-600 text-white flex items-center gap-2 rounded-lg shadow-sm shadow-orange-200">
                    <Plus class="h-4 w-4" /> Tambah Lansia
                </Button>
            </div>
        </template>
            ...
            <div class="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 space-y-6">
                    ...
                    <Card class="border-none shadow-sm bg-white overflow-hidden">
                        <CardContent class="p-4 flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-2xl font-bold text-gray-900 leading-tight">{{ lansia.meta?.total ?? lansia.data.length }}</p>
                            </div>
                        </CardContent>
                    </Card>
                    <Card class="border-none shadow-sm bg-white overflow-hidden">
                    ...
                    <Card class="border-none shadow-sm bg-white overflow-hidden col-span-2 sm:col-span-1">
                        <CardContent class="p-4 flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-2xl font-bold text-gray-900 leading-tight">{{ lansia.data.length - totalAktif }}</p>
                            </div>
                        </CardContent>
                    </Card>
                </div>
                    ...
                    <Table>
                        ...
                        <TableBody>
                            <TableRow v-for="item in lansia.data" :key="item.id"
                                ...
                                @click="router.get(route('lansia.show', { lansia: item.id }))">
                                <TableCell class="pl-5">
                                    <div class="h-9 w-9 rounded-full overflow-hidden bg-orange-50 flex items-center justify-center border border-orange-100">
                                        <img v-if="item.foto" :src="item.foto" :alt="item.nama" class="h-full w-full object-cover" />
                                        <User v-else class="h-4 w-4 text-orange-400" />
                                    </div>
                                </TableCell>
                                <TableCell>
                                ...
                                <TableCell @click.stop>
                                    <DropdownMenu>
                                        ...
                                        <DropdownMenuContent align="end" class="w-44">
                                            <DropdownMenuItem @click="router.get(route('lansia.show', { lansia: item.id }))" class="flex items-center gap-2">
                                                <Eye class="h-4 w-4 text-gray-400" /> Lihat Detail
                                            </DropdownMenuItem>
                                            <DropdownMenuItem @click="router.get(route('lansia.edit', { lansia: item.id }))" class="flex items-center gap-2">
                            ...
                            <TableRow v-if="lansia.data.length === 0">
                                <TableCell colspan="6" class="py-16">
                                    <div class="flex flex-col items-center gap-3 text-center">
                                        <div class="p-4 rounded-full bg-orange-50"><Users class="h-8 w-8 text-orange-300" /></div>
                                        <div>
                                            <p class="font-semibold text-gray-500">Tidak ada data lansia</p>
                                            <p class="text-sm text-muted-foreground mt-0.5">{{ search ? 'Coba ubah kata kunci pencarian.' : 'Mulai dengan mendaftarkan lansia baru.' }}</p>
                                        </div>
                                        <Button v-if="!search" @click="router.get(route('lansia.create'))" variant="outline" size="sm">
                                            <Plus class="h-4 w-4 mr-1" /> Tambah Lansia
                                        </Button>
                                    </div>
                                </TableCell>
                            </TableRow>
                ...
                <div class="flex items-center justify-between text-sm">
                    <p class="text-muted-foreground">
                        Menampilkan <span class="font-semibold text-gray-700">{{ lansia.data.length }}</span>
                        dari <span class="font-semibold text-gray-700">{{ lansia.meta?.total }}</span> data
                    </p>
                    <div class="flex items-center gap-1">
                        <template v-for="link in lansia.meta?.links" :key="link.label">
                            <button v-if="link.url" @click="router.get(link.url, {}, { preserveState: true })" v-html="link.label"
                                :class="['inline-flex h-8 min-w-8 items-center justify-center rounded-lg px-2.5 text-sm transition-colors',
                                    link.active ? 'bg-orange-500 text-white font-semibold shadow-sm' : 'hover:bg-gray-100 text-gray-600']" />
    ...
    <AlertDialog :open="!!deleteTarget" @update:open="(v) => { if (!v) deleteTarget = null }">
        <AlertDialogContent>
            <AlertDialogHeader>
                ...
                <AlertDialogDescription>Tindakan ini tidak dapat dibatalkan. Data lansia akan dihapus secara permanen dari sistem.</AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
