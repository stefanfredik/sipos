...
import { Activity, ArrowLeft, Calendar, Edit, HeartPulse, MapPin, Phone, Trash, User } from 'lucide-vue-next';
...
const deleteLansia = () => {
    if (confirm('Apakah Anda yakin ingin menghapus data ini?')) {
        router.delete(route('lansia.destroy', { lansia: data.id }));
    }
};
</script>
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-4">
                    ...
                    <h2 class="font-semibold text-xl text-gray-800 leading-tight">Detail Lansia</h2>
                </div>
                <div class="flex items-center gap-2">
                    <Link :href="route('lansia.edit', { lansia: data.id })">
                        <Button variant="outline" class="flex items-center gap-2">
                            <Edit class="h-4 w-4" />
                            Edit
                        </Button>
                    </Link>
                    <Button variant="destructive" class="flex items-center gap-2" @click="deleteLansia">
                        <Trash class="h-4 w-4" />
                        Hapus
                    </Button>
                </div>
        ...
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <Card class="md:col-span-2">
                        <CardHeader>
                            ...
                            <CardDescription>Detail lengkap riwayat dan kondisi lansia.</CardDescription>
                        </CardHeader>
                        <CardContent class="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-4">
                            <div class="flex items-start gap-3">
                                <HeartPulse class="h-5 w-5 text-muted-foreground mt-0.5" />
                                <div>
                ...
                <Card>
                    <CardHeader>
                        <CardTitle>Pemantauan Kesehatan Lansia</CardTitle>
                        <CardDescription>Grafik tren berat badan dan tekanan darah.</CardDescription>
                    </CardHeader>
                    <CardContent>
                ...
                <Card>
                    <CardHeader class="flex flex-row items-center justify-between">
                        <div>
                            ...
                            <CardDescription>Semua catatan kunjungan posyandu.</CardDescription>
                        </div>
                        <Link :href="route('pemeriksaan.create', { type: 'lansia', id: data.id })">
                            ...
                            <table class="w-full text-sm">
                                <thead>
                                    <tr class="bg-muted/50 border-b">
                                        ...
                                        <th class="px-4 py-3 text-left font-medium">L. Perut</th>
                                        <th class="px-4 py-3 text-left font-medium">Keluhan</th>
                                ...
                                <tbody>
                                    <tr v-for="item in pemeriksaan.data" :key="item.id" class="border-b transition-colors hover:bg-muted/30">
                                        ...
                                        <td class="px-4 py-3">{{ item.lingkar_perut || '-' }} cm</td>
                                        <td class="px-4 py-3 truncate max-w-[200px]">{{ item.jenis_keluhan || '-' }}</td>
                ...
                <Card v-if="data.keterangan">
                    <CardHeader>
                        <CardTitle>Keterangan Medis / Catatan Tambahan</CardTitle>
                    </CardHeader>
                    <CardContent>
