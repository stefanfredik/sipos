...
import { Download, FileText, Loader2, Filter, HeartPulse, User } from 'lucide-vue-next'

interface IbuHamilData {
    id: string; nik: string; nama: string;
    usia_kehamilan: number | null; no_telp: string; is_active: boolean;
    pemeriksaan: Array<{ lila: number | null }>
}
...
function applyFilters() {
    router.get(route('laporan.ibu-hamil'), {
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="text-xl font-bold tracking-tight text-gray-900">Laporan Ibu Hamil</h2>
                    <p class="text-sm text-muted-foreground mt-0.5">Data ibu hamil terdaftar posyandu</p>
                </div>
                <div class="flex gap-2">
                    <Button variant="outline" size="sm" @click="doExport('laporan.export.pdf', 'ibu_hamil')"
                        :disabled="exporting" class="flex items-center gap-2 rounded-lg border-pink-200 text-pink-700 hover:bg-pink-50">
                        <Loader2 v-if="exporting && exportType === 'pdf'" class="h-4 w-4 animate-spin" />
                        <FileText v-else class="h-4 w-4" />
                        {{ exporting && exportType === 'pdf' ? 'Exporting...' : 'Export PDF' }}
                    </Button>
                    <Button size="sm" @click="doExport('laporan.export.excel', 'ibu_hamil')"
                        :disabled="exporting" class="flex items-center gap-2 rounded-lg bg-pink-600 hover:bg-pink-700 text-white">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
                    <Card class="border-none shadow-sm bg-white overflow-hidden">
                        <CardContent class="p-4 flex items-center gap-3">
                            <div class="p-2.5 rounded-xl bg-gradient-to-br from-pink-500 to-rose-600 text-white shrink-0"><User class="h-5 w-5" /></div>
                            <div>
                    ...
                    <Card class="border-none shadow-sm bg-white overflow-hidden col-span-2 sm:col-span-1">
                        <CardContent class="p-4 flex items-center gap-3">
                            <div class="p-2.5 rounded-xl bg-gray-100 text-gray-500 shrink-0"><User class="h-5 w-5" /></div>
                            <div>
                                <p class="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Tidak Aktif</p>
                                <p class="text-2xl font-bold text-gray-900 leading-tight">{{ data.length - totalAktif }}</p>
                            </div>
                        </CardContent>
                    </Card>
                </div>
                ...
                <Card class="border-none shadow-sm bg-white">
                    <CardHeader class="pb-3 border-b border-gray-100">
                        <CardTitle class="flex items-center gap-2 text-base font-semibold text-gray-700">
                            <div class="p-1.5 rounded-lg bg-pink-50 text-pink-600"><Filter class="h-4 w-4" /></div>
                            Filter
                        </CardTitle>
                    </CardHeader>
                    <CardContent class="p-5">
                        <div class="flex items-end gap-4">
                            ...
                            <Button @click="applyFilters" class="bg-pink-600 hover:bg-pink-700 text-white rounded-lg">Tampilkan</Button>
                        </div>
                    </CardContent>
                </Card>
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow class="bg-gray-50/70 hover:bg-gray-50/70">
                                ...
                                <TableHead class="text-xs font-bold uppercase tracking-wider text-gray-500">No. Telp</TableHead>
                        ...
                        <TableBody>
                            <TableRow v-for="(item, i) in data" :key="item.id" class="hover:bg-gray-50/50">
                                ...
                                <TableCell class="text-center">
                                    <Badge v-if="item.usia_kehamilan" class="bg-pink-50 text-pink-700 border-pink-200 text-xs font-semibold">{{ item.usia_kehamilan }} Minggu</Badge>
                            ...
                            <TableRow v-if="data.length === 0">
                                <TableCell colspan="6" class="py-16">
                                    <div class="flex flex-col items-center gap-3">
                                        <div class="p-3 rounded-full bg-pink-50"><User class="h-6 w-6 text-pink-300" /></div>
                                        <p class="text-sm font-medium text-gray-400">Tidak ada data ibu hamil</p>
                                    </div>
                                </TableCell>
                            </TableRow>
                        </TableBody>
