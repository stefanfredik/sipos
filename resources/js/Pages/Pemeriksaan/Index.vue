...
function deletePemeriksaan(id: string) {
    if (confirm('Apakah Anda yakin ingin menghapus data pemeriksaan ini?')) {
        router.delete(route('pemeriksaan.destroy', id), {
            onSuccess: () => {
                toast.success('Berhasil', 'Data pemeriksaan berhasil dihapus.');
            },
            onError: () => {
        ...
        <div class="space-y-4">
                        ...
                        >
                            ...
                            <TableCell>
                                <Badge variant="outline">{{
                                    getPesertaLabel(item.peserta_type)
                                }}</Badge>
                            </TableCell>
                            <TableCell>
                                <div>
                            ...
                            <TableCell class="text-center">
                                <span
                                    v-if="item.tensi_darah"
                                    class="font-mono text-sm"
                                    >{{ item.tensi_darah }}</span
                                >
                                ...
                                <DropdownMenu>
                                    ...
                                    <DropdownMenuContent align="end">
                                        ...
                                        <DropdownMenuItem
                                            ...
                                            @click="deletePemeriksaan(item.id)"
            ...
            <div class="flex items-center justify-between text-sm">
                <p class="text-muted-foreground">
                    Menampilkan {{ pemeriksaan.data.length }} dari
                    {{ pemeriksaan.meta?.total }} data
                </p>
                <div class="flex items-center gap-1">
                    <Link
                        v-for="link in pemeriksaan.meta?.links"
                        :key="link.label"
                        :href="link.url || '#'"
