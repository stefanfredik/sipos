...
watch(search, debounce((value: string) => {
    router.get(route('posyandu.index'), { search: value }, { preserveState: true, replace: true })
}, 500))
        ...
        <div class="space-y-4">
                ...
                <Table>
                    ...
                    <TableBody>
                        <TableRow v-for="item in posyandu.data" :key="item.id">
                            <TableCell class="font-medium">{{ item.nama_posyandu }}</TableCell>
                            <TableCell>
                            ...
                            <TableCell>
                                <DropdownMenu>
                                    ...
                                    <DropdownMenuContent align="end">
                                        <DropdownMenuItem @click="router.get(route('posyandu.show', { posyandu: item.id }))">Lihat Detail</DropdownMenuItem>
                                        <DropdownMenuItem @click="router.get(route('posyandu.edit', { posyandu: item.id }))">Edit Data</DropdownMenuItem>
                                        <DropdownMenuSeparator />
                                        <DropdownMenuItem class="text-destructive" @click="deletePosyandu(item.id)">Hapus</DropdownMenuItem>
                                    </DropdownMenuContent>
                                </DropdownMenu>
                            </TableCell>
                        </TableRow>
            ...
            <div class="flex items-center justify-between text-sm">
                <p class="text-muted-foreground">
                    Menampilkan {{ posyandu.data.length }} dari {{ posyandu.meta?.total }} data
                </p>
                <div class="flex items-center gap-1">
                    <Link
                        v-for="link in posyandu.meta?.links" :key="link.label" :href="link.url || '#'"
